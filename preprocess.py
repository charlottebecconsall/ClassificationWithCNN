...
def pre_process_data(inputs, labels, first_class, second_class):
  ...
  like 'CIFAR_data_compressed/train' (LOOKS LIKE THESE WERE REPLACED WITH THE ACTUAL FILES)
  ...
  # Remove the extra classes from the dataset
  classes_to_keep = np.array([first_class, second_class])
  data_to_keep = np.isin(labels, classes_to_keep, invert=True)
  indices = np.nonzero(data_to_keep)
  indices = indices[0]
  labels = np.delete(labels, indices)
  inputs = np.delete(inputs, indices, axis=0)

  # Convert labels to binary
  labels[np.nonzero(labels == second_class)] = 0
  labels[np.nonzero(labels == first_class)] = 1
